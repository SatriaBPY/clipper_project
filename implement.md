...
- **Solusi**:
  ...
  6. Menyesuaikan `NEXT_PUBLIC_API_URL` pada service `dashboard` di [docker-compose.yml](file:///D:/vibecoding/clipper_forge/docker-compose.yml) dari `http://localhost:3001` ke `http://localhost:3009` agar frontend dapat terhubung ke backend API.
  7. Meminta user untuk melakukan rebuild dan restart container menggunakan `docker compose build --no-cache` dan `docker compose up -d`.
