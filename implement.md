...
- **Solusi**:
  ...
  10. Memperbarui [worker/src/pipeline/analyze.ts](file:///D:/vibecoding/clipper_forge/worker/src/pipeline/analyze.ts) agar menyertakan respons JSON lengkap dari NineRouter ketika mendeteksi content kosong, guna mempermudah penelusuran masalah (debugging).
  11. Menginstal `fontconfig`, `fonts-liberation`, dan `fonts-dejavu-core` di [worker/Dockerfile](file:///D:/vibecoding/clipper_forge/worker/Dockerfile) serta menjalankan `fc-cache -fv` karena image debian-slim tidak memiliki font sistem bawaan. Hal ini memulihkan tampilan teks subtitle agar tidak kosong/hilang saat proses rendering oleh `ffmpeg`.
  12. Mengimplementasikan gaya tulisan dan efek transisi kata aktif CapCut di [worker/src/pipeline/caption.ts](file:///D:/vibecoding/clipper_forge/worker/src/pipeline/caption.ts) dengan mewarnai kata aktif menjadi kuning (`&H0000FFFF&`) sementara kata lainnya tetap putih, memperbesar ukuran font menjadi 38, serta menaikkan margin vertikal ke 120 agar tidak terpotong oleh UI media sosial.
  13. Meminta user untuk melakukan rebuild dan restart container menggunakan `docker compose build --no-cache` dan `docker compose up -d`.
